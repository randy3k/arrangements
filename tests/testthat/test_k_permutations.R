...
test_that("K-Permutations - ipermutations", {
    ...
    expect_equal(iperm$getnext(type = "c"), 1:3)
    expect_equal(iperm$getnext(type = "c"), c(1, 2, 4))
    ...
    expect_equal(iperm$getnext(type = "l"), 1:3)
    expect_equal(iperm$getnext(type = "l"), c(1, 2, 4))
...
test_that("Multiset K-Permutations - npermutations", {
    expect_equal(npermutations(f = c(3, 2, 3), k = 4), 70)
    expect_equal(npermutations(x = LETTERS[1:3], f = c(3, 2, 3), k = 4), 70)
    expect_error(npermutations(f = c(10, 50, 10), k = 20), "integer overflow")
    expect_error(npermutations(x = LETTERS[1:3], f = c(10, 50, 10), k = 20), "integer overflow")
    expect_equal(npermutations(f = c(10, 50, 10), k = 20, bigz = TRUE), gmp::as.bigz("3224323183"))
    expect_equal(npermutations(f = c(0, 0, 0), k = 4), 0)
    expect_equal(npermutations(f = c(0, 0, 0), k = 0), 1)
    expect_error(npermutations(f = c(3, 2, 3), k = -1), "expect non-negative integer")
    expect_error(npermutations(f = c(3, 2, 3), k = 1.5), "expect non-negative integer")
})

test_that("Multiset K-Permutations - permutations", {
    perm <- permutations(f = c(3, 2, 3), k = 4)
    expect_equal(nrow(perm), 70)
    expect_equal(ncol(perm), 4)
    expect_equal(perm[1, ], c(1, 1, 1, 2))
    expect_equal(perm[70, ], c(3, 3, 3, 2))

    perm <- permutations(f = c(3, 2, 3), k = 4, type = "c")
    expect_equal(ncol(perm), 70)
    expect_equal(nrow(perm), 4)
    expect_equal(perm[, 1], c(1, 1, 1, 2))
    expect_equal(perm[, 70], c(3, 3, 3, 2))

    perm <- permutations(f = c(3, 2, 3), k = 4, type = "l")
    expect_equal(length(perm), 70)
    expect_equal(perm[[1]], c(1, 1, 1, 2))
    expect_equal(perm[[70]], c(3, 3, 3, 2))

    perm <- permutations(x = LETTERS[1:5], f = c(3, 2, 3), k = 4)
    expect_equal(nrow(perm), 70)
    expect_equal(ncol(perm), 4)
    expect_equal(perm[1, ], LETTERS[c(1, 1, 1, 2)])
    expect_equal(perm[70, ], LETTERS[c(3, 3, 3, 2)])

    expect_error(permutations(f = c(10, 50, 10), k = 20), "too many results")
    expect_error(permutations(f = c(3, 2, 3), k = -1), "expect non-negative integer")
    expect_error(permutations(f = c(3, 2, 3), k = 1.5), "expect non-negative integer")
    expect_equal(dim(permutations(f = c(3, 2, 3), k = 0)), c(1, 0))
    expect_equal(dim(permutations(f = c(0, 0, 0), k = 1 )), c(0, 1))
    expect_equal(dim(permutations(f = c(0, 0, 0), k = 0 )), c(1, 0))
})

test_that("Multiset K-Permutations - ipermutations", {
    iperm <- ipermutations(f = c(3, 2, 3), k = 4)
    perm <- permutations(f = c(3, 2, 3), k = 4)
    expect_equal(iperm$collect(), perm)
    ...
    perm <- permutations(f = c(3, 2, 3), k = 4, type = "c")
    expect_equal(iperm$collect(type = "c"), perm)
    expect_equal(iperm$getnext(type = "c"), c(1, 1, 1, 2))
    expect_equal(iperm$getnext(type = "c"), c(1, 1, 1, 3))
    ...
    perm <- permutations(f = c(3, 2, 3), k = 4, type = "l")
    expect_equal(iperm$collect(type = "l"), perm)
    expect_equal(iperm$getnext(type = "l"), c(1, 1, 1, 2))
    expect_equal(iperm$getnext(type = "l"), c(1, 1, 1, 3))
    iperm$getnext(60, type = "l")
    expect_equal(length(iperm$getnext(10, type = "l")), 8)
    expect_equal(iperm$getnext(type = "l"), NULL)

    expect_error(ipermutations(f = c(3, 2, 3), k = -1), "expect non-negative integer")
    expect_error(ipermutations(f = c(3, 2, 3), k = 1.5), "expect non-negative integer")
    iperm <- ipermutations(f = c(3, 2, 3), k = 0)
    expect_equal(dim(iperm$collect()), c(1, 0))
    expect_equal(length(iperm$getnext()), 0)
    expect_equal(iperm$getnext(), NULL)
    iperm <- ipermutations(f = c(3, 2, 3), k = 9)
    expect_equal(iperm$collect(), NULL)
    expect_equal(iperm$getnext(), NULL)
})
