            ...
            if (k_rem == 0) {
                ...
                if (n_new >= 0) {
                    if (n_new == 0) val = 1;
                        ...
                        int k_idx = k_rem < n_new ? k_rem : n_new;
                        int m_idx = m_new < n_new ? m_new : n_new;
                        val = table[n_new * n2 + k_idx * n1 + m_idx];
                    }
                }
            }
    ...
    for (i = 0; i < k; i++) {
        ...
        for (j = start; j >= 1; j--) {
            ...
            if (k_rem == 0) {
                ...
                int m_new = j - 1;
                if (n_new >= 0) {
                    if (n_new == 0) mpz_add_ui(this_count, count, 1);
                    else {
                        int k_idx = k_rem < n_new ? k_rem : n_new;
                        int m_idx = m_new < n_new ? m_new : n_new;
                        mpz_add(this_count, count, table[n_new * n2 + k_idx * n1 + m_idx]);
                    }
                } else {
...
SEXP draw_desc_k_partitions(int n, int k, char layout, SEXP _index, SEXP _nsample) {
    ...
    if (bigz) {
        ...
        #define NEXT() \
            ...
            nth_desc_k_partition_table_bigz(ap, n, k, z, table, n);
        ...
        #define NEXT() \
            if (sampling) { \
                nth_desc_k_partition_table(ap, n, k, floor(maxd * unif_rand()), table, n); \
            } else { \
                nth_desc_k_partition_table(ap, n, k, index[j] - 1, table, n); \
            }
