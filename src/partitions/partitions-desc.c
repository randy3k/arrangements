...
void nth_desc_partition(unsigned int* ar, unsigned int n, unsigned int index) {
    ...
    make_partition_max_table(table, n);
    nth_desc_partition_table(ar, n, index, table, n);
    free(table);
}
...
void nth_desc_partition_bigz(unsigned int* ar, unsigned int n, mpz_t index) {
    ...
    make_partition_max_table_bigz(table, n);
    nth_desc_partition_table_bigz(ar, n, index, table, n);
    int i;
...
void nth_desc_partition_table(unsigned int* ar, unsigned int n, unsigned int index, double* table, int table_n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (sum > 0 && i < n - 1) {
            for (j = start; j >= 1; j--) {
                if (sum < j) continue;
                int idx = j < sum - j ? j : sum - j;
                this_count = count + table[(sum - j) * (table_n + 1) + idx];
    ...
    for (i = 0; i < n; i++) {
        ...
        if (sum > 0 && i < n - 1) {
            for (j = start; j >= 1; j--) {
                if (sum < j) continue;
                int idx = j < sum - j ? j : sum - j;
                mpz_add(this_count, count, table[(sum - j) * (table_n + 1) + idx]);
...
SEXP draw_desc_partitions(int n, char layout, SEXP _index, SEXP _nsample) {
    int i, j;
    int nprotect = 0;
    int bigz = 0;
    ...
    if (bigz) {
        ...
        #define NEXT() \
            ...
            nth_desc_partition_table_bigz(ap, n, z, table, n); \
            for (i = 0; i < n; i++) { \
                if (ap[i] == 0) { \
                    break; \
                } \
        ...
        #define NEXT() \
            if (sampling) { \
                nth_desc_partition_table(ap, n, floor(maxd * unif_rand()), table, n); \
            } else { \
                nth_desc_partition_table(ap, n, index[j] - 1, table, n); \
            } \
            for (i = 0; i < n; i++) { \
                if (ap[i] == 0) { \
                    break; \
